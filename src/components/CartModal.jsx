...
const CartModal = ({ product, close, quantity }) => {
  ...
  const putCart = async () => {
    ...
    try {
      await axios.post(`https://whippedback.9seebird.site/add_item/${userId}`, cartItem, {
                ...
                src={
                  item.product_size
                    ? item.product_size === '80g'
                      ? item.details?.p_main_img
                      : item.details?.p_main_img_1
                    : item.details?.p_main_img
                }
              />
            </div>
                ...
                {item.product_size
                  ? item.product_size === '80g'
                    ? new Intl.NumberFormat().format(
                        (parseInt(item.details?.p_price?.replace(/,/g, ''), 10) || 0) *
                          (parseInt(item.quantity, 10) || 0)
                      )
                    : new Intl.NumberFormat().format(
                        (parseInt(item.details?.p_price_1?.replace(/,/g, ''), 10) || 0) *
                          (parseInt(item.quantity, 10) || 0)
                      )
                  : new Intl.NumberFormat().format(
