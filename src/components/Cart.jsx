...
const Cart = () => {
  ...
  useEffect(() => {
    const fetchCarts = async () => {
      try {
        const response = await axios.get(`https://whippedback.9seebird.site/get_cart/${userId}`);
        const { cartItems, diyItems } = response.data;
        const productRequests = cartItems.map((item) =>
          axios.get(`https://whippedback.9seebird.site/get_detail_products/${item.product_id}`)
        );
  ...
  const calculateSelectedCartsPrice = (checkItems) => {
    const total = checkItems.reduce((sum, item) => {
      const itemPriceKey =
        item.productSize === 'null' ? 'p_price' : item.productSize === '130g' ? 'p_price_1' : 'p_price';
  ...
  const quantityChange = async (userId, productId, productSize, newQuantity, cartItemId) => {
    try {
      ...
      // 수량 업데이트 요청
      await axios.post(`https://whippedback.9seebird.site/update_quantity/${userId}`, requestBody);
  ...
  const noItem = async (userId, productId, productSize, cartItemId) => {
    try {
      ...
      if (isDiyItem) {
        // DIY 아이템 삭제 요청
        await axios.post(`https://whippedback.9seebird.site/delete_item/${userId}`, { cartItemId });
      } else {
        // 일반 아이템 삭제 요청
        await axios.post(`https://whippedback.9seebird.site/delete_item/${userId}`, { productId, productSize });
      }
      ...
      const productRequests = updatedCarts
        ...
        .map((item) => axios.get(`https://whippedback.9seebird.site/get_detail_products/${item.product_id}`));
  ...
  const deleteAll = async () => {
    try {
      const response = await axios.post(`https://whippedback.9seebird.site/delete_all/${userId}`);
  ...
  const deleteCheckedItems = async () => {
    try {
      ...
      // 서버에 삭제 요청 전송
      await axios.post(`https://whippedback.9seebird.site/delete_checked_items/${userId}`, { items: itemsToDelete });

      // 삭제 후 장바구니 데이터 가져오기
      const updatedCartResponse = await axios.get(`https://whippedback.9seebird.site/get_cart/${userId}`);
        ...
        <div className="cart_content">
          <div className="map_wrapper">
            <div>
              ...
              {isNormal ? (
                <>
                  <div className="cart_normal">
                    일반상품 ({carts.length + diyItem.length})
                    <div className="input_wrapper">
                      <input
                        ...
                        checked={checkItems.length === carts.length + diyItem.length ? true : false}
                      />
                    </div>
                  </div>
                  <div className="cart_normal_2_wrapper">
                    <div className="cart_normal_2" style={{ order: '3' }}>
                      <button onClick={deleteAll} className="delete_all_btn">
                        전체 삭제
                      </button>
                    </div>
                    <div className="cart_normal_2" style={{ order: '2' }}>
                      <button onClick={deleteCheckedItems} className="delete_all_btn">
                        선택 삭제
                      </button>
                    </div>
                  </div>
            ...
            <div className="All">{/* <button  className="deleteAll_btn">전체 삭제</button> */}</div>
            {carts.map((cart, idx) => {
                ...
                return (
                  <div className="cart_products">
                    ...
                    <div className="cart_right">
                      <div className="cart_up">
                        <div className="cart_up_txt">
                          <Link to={'/diyItem'} style={{ textDecoration: 'none', color: 'black' }}>
